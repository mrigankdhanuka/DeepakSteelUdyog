...
export const AIChatBot = () => {
  ...
  // Initialize chat session with ONLY available products
  useEffect(() => {
    // 1. Filter out-of-stock items
    const availableProducts = products.filter(p => {
        // Calculate total stock (sum of variants or global stock)
    ...
    // 2. Initialize chat if we have products and no session yet
    if (availableProducts.length > 0 && !chatSessionRef.current) {
      chatSessionRef.current = geminiService.createShoppingAssistantChat(availableProducts);
    }
  }, [products]);
  ...
  const handleSend = async () => {
    ...
    // If chat isn't ready (e.g., failed to fetch products), handle gracefully
    if (!chatSessionRef.current) {
        setMessages(prev => [...prev, { role: 'user', text: input }]);
        setTimeout(() => {
            setMessages(prev => [...prev, { role: 'model', text: "I'm still connecting to the inventory system. Please wait a moment." }]);
        }, 500);
        return;
    }
