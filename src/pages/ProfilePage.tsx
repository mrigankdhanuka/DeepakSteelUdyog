...
import { User, LogOut, Package, MapPin, Heart, Settings, Camera, Plus, Trash2, Home, Briefcase, CheckCircle2, AlertCircle, Lock, ShoppingBag, Download, RefreshCcw, Loader2 } from 'lucide-react';
...
export const ProfilePage = () => {
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    e.preventDefault();
    await updateUserProfile({ name: profileData.name, phoneNumber: profileData.phone });
                  ...
                  <div className="bg-gray-50 p-6 rounded-xl mb-8 animate-in slide-in-from-top-4">
                    ...
                    <form onSubmit={handleSaveAddress} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="md:col-span-2 flex gap-3 mt-2">
                         ...
                         <button type="submit" disabled={isSavingAddress} className="px-4 py-2 bg-blue-600 text-white rounded-lg font-bold disabled:opacity-50 flex items-center gap-2">
                            {isSavingAddress ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Save Address'}
                         </button>
                      </div>
                    </form>
                  </div>
                ...
                {wishlistProducts.length > 0 ? (
                   <div className="space-y-4">
                      {wishlistProducts.map(product => {
                         ...
                         // Simplified check for overall stock (sum of all variants OR base stock if variants empty)
