...
export const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    <div className={`bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:shadow-blue-100 transition-all duration-300 group flex flex-col h-full relative ${isOutOfStock ? 'opacity-75' : ''}`}>
      ...
      >
        ...
        {isOutOfStock && (
          <div className="absolute inset-0 bg-white/60 backdrop-blur-[2px] flex items-center justify-center z-30">
             <div className="bg-gray-900 text-white px-4 py-2 rounded-lg font-bold shadow-lg flex items-center gap-2">
        ...
        {product.discountPrice && !isOutOfStock && (
          <div className="absolute top-2 left-2 bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold px-2 py-1 rounded-md z-20 shadow-sm">
            SALE
          </div>
        )}
        ...
        <button 
          ...
          className={`absolute top-2 right-2 p-2 rounded-full z-20 shadow-sm transition-colors ${
             isWishlisted ? 'bg-white text-red-500' : 'bg-white/80 text-gray-400 hover:text-red-500'
          }`}
        >
      ...
      <div className="p-4 flex-grow flex flex-col">
        <div className="mb-2">
          ...
          <Link to={`/product/${product.id}`}>
             <h3 className="text-gray-900 font-semibold text-lg leading-tight mt-2 line-clamp-2 group-hover:text-blue-700 transition-colors">{product.title}</h3>
          </Link>
        </div>
          ...
          <button 
            ...
            className={`p-3 rounded-xl transition-all duration-300 shadow-sm ${
              isOutOfStock 
                ...
                : 'bg-blue-50 text-blue-600 hover:bg-gradient-to-r hover:from-blue-600 hover:to-blue-500 hover:text-white hover:shadow-lg hover:shadow-blue-500/30'
            }`}
