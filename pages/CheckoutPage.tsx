...
export const CheckoutPage = () => {
  ...
  const serviceablePincodes = ['302001', '332301', '110001', '400001']; // Mock Pincodes
  ...
  // Use discountAmount from context which is already calculated based on rules
  ...
  const processOrderCreation = async () => {
    ...
    setPaymentError('');
    setShowCodOtp(false); // Close modal if open
    ...
    try {
      // 2. Mock Order Creation (This now includes rate limiting)
      const orderId = await placeOrder(orderData);
      setCurrentOrderId(orderId);

      // 3. Simulate Payment Gateway Delay
      setTimeout(() => {
         // Random Payment Failure Simulation (80% success rate for demo)
         // For COD, always success if OTP passed
         const isSuccess = paymentMethod === 'COD' ? true : Math.random() > 0.2; 
         
         if (isSuccess) {
            handlePaymentResult(orderId, true);
            setOrderSuccessId(orderId);
            setIsProcessing(false);
         } else {
            handlePaymentResult(orderId, false);
            setPaymentError('Payment declined by bank. Please try again.');
            setIsProcessing(false);
            // Restore cart so user can try again
            restoreCartFromOrder(orderId);
         }
      }, 2000);

    } catch (error: any) {
      console.error(error);
      setPaymentError(error.message || 'Failed to initialize order. Please try again.');
      setIsProcessing(false);
    }
  };

  const verifyCodOtpAndPlaceOrder = (e: React.FormEvent) => {
    e.preventDefault();
    if (codOtp === '1234') {
     ...
     return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] animate-in fade-in">
           <div className="relative">
             ...
             <ShieldCheck className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-600 h-8 w-8" />
           </div>
           <h2 className="text-2xl font-bold text-gray-900 mt-6">Processing Payment</h2>
           <p className="text-gray-500 mt-2">Please do not refresh the page...</p>
           <div className="mt-8 p-4 bg-blue-50 rounded-lg max-w-md text-sm text-blue-700 flex items-center gap-3">
              <Lock className="h-4 w-4" /> Securely communicating with bank servers.
           </div>
        </div>
     );
  }
              ...
              <div className="pl-10 animate-in fade-in space-y-4">
                 <label className={`flex items-center gap-4 p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'Card' ? 'border-blue-600 bg-blue-50' : 'border-gray-200 hover:border-blue-300'}`}>
                    ...
                    <div className="flex-1">
                       <p className="font-bold text-gray-900 flex items-center gap-2"><CreditCard className="h-5 w-5" /> Credit / Debit Card / UPI</p>
                 ...
                 <div className="pt-6">
                    ...
                    <p className="text-center text-xs text-gray-400 mt-4 flex items-center justify-center gap-1">
                       <Lock className="h-3 w-3" /> 256-bit SSL Encrypted Payment
                    </p>
                 </div>
              </div>
           )}
