...
import { Package, Users, DollarSign, TrendingUp, ShoppingBag, Plus, Edit, Trash, Box, Clock, Truck, CheckCircle, XCircle, Tag, Upload, Eye, X, ChevronDown, ChevronUp, AlertTriangle, Save, RefreshCw } from 'lucide-react';
import { MOCK_ANALYTICS } from '../mockData';
import { Product, OrderStatus, Order, ProductVariant, PaymentStatus, Coupon } from '../types';

// ... StatCard and ProductModal components remain same ...
...
const ProductModal = ({ isOpen, onClose, product, onSave }: { isOpen: boolean, onClose: () => void, product?: Product, onSave: (data: any) => void }) => {
  ...
  React.useEffect(() => {
    if (isOpen && !product) {
       setFormData({
        title: '', price: 0, category: 'Almirah', stock: 0, description: '', image: 'https://images.unsplash.com/photo-1595428774223-ef52624120d2?auto=format&fit=crop&w=800', variants: [], tags: []
       });
       setTagInput('');
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-blue-900/40 backdrop-blur-sm">
      <div className="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="grid grid-cols-2 gap-4">
             ...
             <div className="col-span-2">
                ...
                <div className="flex gap-2">
                  <input className="flex-1 border border-gray-300 p-2.5 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="https://..." value={formData.image} onChange={e => setFormData({...formData, image: e.target.value})} />
                  <div className="w-10 h-10 rounded border border-gray-200 bg-gray-50 overflow-hidden flex-shrink-0">
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
          ...
          <button onClick={() => onSave(formData)} className="px-5 py-2.5 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 shadow-md">Save Product</button>
        </div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-blue-900/40 backdrop-blur-sm animate-in fade-in">
       <div className="bg-white rounded-xl w-full max-w-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="p-6 overflow-y-auto">
             {/* ... (Keeping existing structure for customer details, address, etc) ... */}
...
export const AdminDashboard = () => {
  const { products, orders, user, addProduct, updateProduct, deleteProduct, updateOrderStatus, coupons, addCoupon, updateCoupon, deleteCoupon, lowStockItems } = useStore();
    ...
    <div className="space-y-8">
      ...
      {activeTab === 'overview' && (
        <div className="space-y-8 animate-in fade-in">
           {/* ... Stat Cards and Low Stock Alerts (keeping same) ... */}
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
             <StatCard title="Total Revenue" value="$45,231.89" icon={DollarSign} color="bg-blue-600" />
             ...
             <StatCard title="Active Customers" value="8,540" icon={Users} color="bg-sky-500" />
           </div>
             ...
             <table className="w-full text-left text-sm text-gray-500">
               ...
               <tbody className="divide-y divide-gray-100">
                 {products.map(product => {
                    const totalStock = product.variants ? product.variants.reduce((acc, v) => acc + v.stock, 0) : product.stock;
                    return (
